...
# VAD_THRESHOLD = float(os.getenv("VAD_THRESHOLD", "3"))  # e.g., stops after ~1.2s silence smaller = faster detection
...
# The main service function to be called by your StateGraph
# -------------------------------
def stt_service(state: VoiceState) -> dict:  # ✅ Fixed: LangGraph signature and return type
    """
    Start a short real-time STT session, capture one finalized utterance, append it
    to state messages as a HumanMessage, and return the updated state.
    """
    timeout = DEFAULT_STT_TIMEOUT  # Use default timeout from env
    ...
    # -------------------------------
    # Create client and register callbacks (FIXED: Only create once!)
    # -------------------------------
    client = StreamingClient(
        StreamingClientOptions(
            api_key=API_KEY,
    ...
    if not utter_final:
        # No speech captured within timeout
        logger.info("No speech captured within timeout.")
        return {}  # ✅ Fixed: Return empty dict, not state

    print("\n🟢 Final utterance captured:")
    print(utter_final)

    # ✅ Fixed: Return dict to update state, ensure messages exist
    try:
